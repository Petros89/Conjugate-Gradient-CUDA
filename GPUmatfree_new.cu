...
__global__ void GPUMatVec(double *Au, double *Ke, double *U, double *r, size_t numElems, int *edofMat)

{
  //global gpu thread index
	//double Au;
  int i = threadIdx.x;
  size_t e = blockIdx.x * BLOCKSIZE + i;

  if (e<numElems)
  {

	for (int i=0; i<MNE; i++){  
        Au[edofMat[i+MNE*e]] = Ke[i*MNE+0]*U[edofMat[0+MNE*e]] + Ke[i*MNE+1]*U[edofMat[1+MNE*e]] + Ke[i*MNE+2]*U[edofMat[2+MNE*e]] + Ke[i*MNE+3]*U[edofMat[3+MNE*e]] +
             Ke[i*MNE+4]*U[edofMat[4+MNE*e]] + Ke[i*MNE+5]*U[edofMat[5+MNE*e]] + Ke[i*MNE+6]*U[edofMat[6+MNE*e]] + Ke[i*MNE+7]*U[edofMat[7+MNE*e]]; 

    
        atomicAdd(&r[edofMat[i+MNE*e]], Au[edofMat[i+MNE*e]]);
	}
  }
}
...
/*------let's let the NVIDIA to take care allocation of matrices using the Unified Memory API----------*/
  ...
  int *face_d, *edofMat_d, *ndofMat_d;
  double *U_d, *r_d, *p_d; 
  double *Ap_d, *Ke_d; //z_d
  ...
  //double *TEMP = (double *) malloc(numNodes * sizeof(double));
  ...
  cudaMalloc((void **) &ndofMat_d, MNE*numElems * sizeof(int));
  ...
  cudaMallocManaged((void **) &Au, (numElems*MNE) * sizeof(double));
  ...
  for(size_t e=0; e<numElems; e++)
  {
    ...
    cpt = 0.0; while (ndofMat[cpt + MNE*edofMat[0 + MNE*e]] >= 0); cpt++; ndofMat[cpt + MNE*edofMat[0 + MNE*e]] = e; 
    cpt = 0.0; while (ndofMat[cpt + MNE*edofMat[1 + MNE*e]] >= 0); cpt++; ndofMat[cpt + MNE*edofMat[1 + MNE*e]] = e; 
    cpt = 0.0; while (ndofMat[cpt + MNE*edofMat[2 + MNE*e]] >= 0); cpt++; ndofMat[cpt + MNE*edofMat[2 + MNE*e]] = e; 
    cpt = 0.0; while (ndofMat[cpt + MNE*edofMat[3 + MNE*e]] >= 0); cpt++; ndofMat[cpt + MNE*edofMat[3 + MNE*e]] = e; 
    cpt = 0.0; while (ndofMat[cpt + MNE*edofMat[4 + MNE*e]] >= 0); cpt++; ndofMat[cpt + MNE*edofMat[4 + MNE*e]] = e; 
    cpt = 0.0; while (ndofMat[cpt + MNE*edofMat[5 + MNE*e]] >= 0); cpt++; ndofMat[cpt + MNE*edofMat[5 + MNE*e]] = e; 
    cpt = 0.0; while (ndofMat[cpt + MNE*edofMat[6 + MNE*e]] >= 0); cpt++; ndofMat[cpt + MNE*edofMat[6 + MNE*e]] = e; 
    cpt = 0.0; while (ndofMat[cpt + MNE*edofMat[7 + MNE*e]] >= 0); cpt++; ndofMat[cpt + MNE*edofMat[7 + MNE*e]] = e; 
  }

  for (int i=0; i<numNodes; i++) printf(" ndof = %d\n", ndofMat[i]);
  ...
  //for(size_t n=0; n<numNodes; n++)    z[n] = r[n]; // NO PRECONDITIONNER
  ...
  cudaMemcpy(ndofMat_d, ndofMat, (numElems*MNE) * sizeof(int),cudaMemcpyHostToDevice); 
  ...
  for(iter=0; iter<Max_iters; iter++)
  {
    ...
    GPUMatVec<<<NBLOCKS,BLOCKSIZE>>>(Au, Ke_d, p_d, Ap_d, numElems, edofMat_d);
    //cudaDeviceSynchronize();  
